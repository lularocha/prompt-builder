...
| **react-markdown + remark-gfm** | Formatted Markdown preview modal (GFM tables, lists)                                               |
| **docx**                        | Generate Word (`.docx`) export from the prompt's Markdown                                          |
...
- `lucide-react` - Icons
- `react-markdown`, `remark-gfm` - Markdown rendering for the preview modal
...
| `PROMPT_RULES.md`                        | Source-of-truth rules for prompt generation (mirrored in `route.ts`)     |
| `app/api/generate-prompt/route.ts`       | AI prompt-generation endpoint (provider-agnostic) + error classification |
| `components/prompt-builder.tsx`          | Main state container, generation orchestration, error banner             |
...
| `components/generated-prompt.tsx`        | Editable output box, preview modal, copy / .md / .docx export            |
| `components/markdown-preview.module.css` | Notion-style styles for the preview modal                                |
...
| `lib/markdown-to-docx.ts`                | Converts the prompt Markdown to a Word `.docx`                           |

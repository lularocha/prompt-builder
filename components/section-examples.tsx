...
const SUGGESTION_GROUPS: { key: SuggestionCategory; label: string }[] = [
  { key: "constraints", label: "Behavior & Constraints" },
  { key: "task", label: "Task" },
      ...
      <CardContent className="space-y-4">
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">
            Images (PNG, JPG), PDFs, Code files, or Text files
          </p>
        </div>
        ...
        <div className="p-4 rounded-lg bg-blue-500/10 space-y-3">
          <div className="flex items-center justify-between">
            ...
            {analysisStatus === "idle" && (
              <Badge
                variant="secondary"
                className="bg-gray-600 text-white border-0 text-sm px-3.5 py-1"
              >
            ...
            {analysisStatus === "analyzing" && (
              <Badge
                variant="secondary"
                className="border-0 text-black text-sm px-3.5 py-1 animate-glow-orange"
              >
                <BrainCircuit className="w-3 h-3 mr-1" />
                Analyzing...
              </Badge>
            ...
            {analysisStatus === "complete" && (
              <Badge
                variant="secondary"
                className="bg-green-600 text-white border-0 text-sm px-3.5 py-1"
              >
                <BrainCircuit className="w-3 h-3 mr-1" />
                Complete
              </Badge>
          ...
          {!visualContextSuggestions && (
            <>
              <div className="text-sm text-white font-bold">
                Upload examples to have the agent extract patterns and make
                suggestions for Behavior & Constraints, Task, Requirements, and
                Tech fields below.
              </div>
          ...
          {visualContextSuggestions && (
            <div className="space-y-6 pt-2">
              ...
              <div className="space-y-4">
                ...
                {SUGGESTION_GROUPS.filter(
                  ({ key }) =>
                    key === "task" || key === "requirements" || key === "tech",
                ).map(({ key, label }) => (
                  <div key={key}>
                    <h4 className="text-xl font-semibold text-blue-400 mb-2">
                      {label}
        ...
        <div className="space-y-3">
          <div className="flex flex-col gap-1">
            <h4 className="text-xl font-bold text-blue-400 mt-12 block">
              Paste Code Snippets
            </h4>
            <span className="block text-sm text-muted-foreground">
              Demo usage or existing code patterns.
            </span>
          </div>
          <Textarea
